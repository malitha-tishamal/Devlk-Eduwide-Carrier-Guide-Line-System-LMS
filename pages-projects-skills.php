...
if (isset($_POST['submit_skills'])) {
    ...
    if (empty($selected_skills)) {
        ...
        foreach ($selected_skills as $skill_id) {
            // Verify the skill exists and get its category
            $check_skill = $conn->prepare("
                SELECT id, category FROM (
                    SELECT id, 'IT' AS category FROM it_student_skills
                    UNION ALL
                    SELECT id, 'Engineering' AS category FROM engineering_skills
                ) AS all_skills WHERE id = ?
            ");
            $check_skill->bind_param("i", $skill_id);
            $check_skill->execute();
            $skill_exists = $check_skill->get_result()->fetch_assoc();
            $check_skill->close();
        ...
        if ($added_count > 0) {
            $skills_success = "$added_count skill(s) added successfully!";
        } else {
            $skills_error = "No new skills were added. They may already be in your profile.";
        }
...
if (isset($_POST['submit_project'])) {
    ...
    $end_date = $_POST['end_date'] ?: null; // Optional end date
    
    // Handle project links
    $links = [];
    for ($i = 1; $i <= 4; $i++) {
        $link_type = trim($_POST["link_type_$i"] ?? '');
        $link_url = trim($_POST["link_url_$i"] ?? '');
    ...
    if (empty($title) || empty($description) || empty($start_date)) {
        $project_error = "Title, description, and start date are required!";
    } else {
        // Validate dates
                    ...
                    <div class="card-body">
                        ...
                        <form method="post" id="skillsForm">
                            ...
                            <div class="skill-container" id="skillsList">
                                ...
                                $added_skill_ids = array_column($added_skills, 'id');
                                ...
                                <?php endif; ?>
                                    <label class="skill-checkbox">
                                        <input type="checkbox" name="skills[]" value="<?= $skill['id'] ?>" 
                                            <?= in_array($skill['id'], $added_skill_ids) ? 'disabled' : '' ?>>
                                        <?= htmlspecialchars($skill['skill_name']) ?>
                                        <?php if(in_array($skill['id'], $added_skill_ids)): ?>
                            ...
                            <div class="mt-3 d-flex justify-content-between align-items-center">
                                ...
                                <button type="submit" class="btn btn-primary" id="submitSkillsBtn">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form method="post" enctype="multipart/form-data" id="projectForm">
                            ...
                            <!-- Project Links -->
                            <div class="mb-3">
                                <label class="form-label fw-bold">Project Links (Max 4)</label>
                                <small class="link-counter d-block">Add up to 4 links with descriptions</small>
                                
                                <div id="linksContainer">
                                    <!-- Link 1 -->
                                    <div class="link-row">
                                        <div class="link-type">
                                            <input type="text" name="link_type_1" class="form-control" placeholder="Link type (e.g., GitHub)" maxlength="50">
                                        </div>
                                        <div class="link-url">
                                            <input type="url" name="link_url_1" class="form-control" placeholder="https://...">
                                        </div>
                                    </div>
                                    <!-- Link 2 -->
                                    <div class="link-row">
                                        <div class="link-type">
                                            <input type="text" name="link_type_2" class="form-control" placeholder="Link type (e.g., Demo)" maxlength="50">
                                        </div>
                                        <div class="link-url">
                                            <input type="url" name="link_url_2" class="form-control" placeholder="https://...">
                                        </div>
                                    </div>
                                    <!-- Link 3 -->
                                    <div class="link-row">
                                        <div class="link-type">
                                            <input type="text" name="link_type_3" class="form-control" placeholder="Link type (e.g., Documentation)" maxlength="50">
                                        </div>
                                        <div class="link-url">
                                            <input type="url" name="link_url_3" class="form-control" placeholder="https://...">
                                        </div>
                                    </div>
                                    <!-- Link 4 -->
                                    <div class="link-row">
                                        <div class="link-type">
                                            <input type="text" name="link_type_4" class="form-control" placeholder="Link type (e.g., Website)" maxlength="50">
                                        </div>
                                        <div class="link-url">
                                            <input type="url" name="link_url_4" class="form-control" placeholder="https://...">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="project-card">
                                <div class="project-header">
                                    <div>
                                        ...
                                        <div class="project-timeline mt-1">
                                            <small class="text-muted">
                                                ...
                                                <?php if($project['end_date']): ?>
                                                    ...
                                                    <?php 
                                                        ...
                                                        echo '<span class="badge bg-light text-dark ms-2">' . $duration->format('%m months %d days') . '</span>';
                                                    ?>
                                                <?php else: ?>
                                                    - Present
                                                    <span class="badge bg-info ms-2">Ongoing</span>
                                                <?php endif; ?>
                                            </small>
                                        </div>
                                    </div>
                                    ...
                                    <div class="project-links mb-2">
                                        <?php while($link = $links_result->fetch_assoc()): ?>
                                            <div class="project-link-item">
                                                ...
                                                <a href="<?= htmlspecialchars($link['link_url']) ?>" target="_blank" class="text-decoration-none">
                                                    View <?= htmlspecialchars($link['link_type']) ?>
                                                </a>
                                            </div>
                                        <?php endwhile; ?>
                                    </div>
                                <?php endif; ?>
                                <?php $links_stmt->close(); ?>
                                
                                <div class="project-meta">
                                    <small><i class="far fa-calendar me-1"></i>Added: <?= date('M j, Y', strtotime($project['created_at'])) ?></small>
                                    <small><i class="far fa-image me-1"></i><?= $project['image_count'] ?> images</small>
                                    <small><i class="fas fa-link me-1"></i><?= $project['link_count'] ?> links</small>
                                </div>
...
document.addEventListener("DOMContentLoaded", function(){
    ...
    const submitSkillsBtn = document.getElementById('submitSkillsBtn');
    ...
    function updateSelectedCount() {
        ...
        submitSkillsBtn.disabled = checkedCount === 0;
    }
    ...
    projectForm.addEventListener('submit', function(e) {
        ...
        let validLinks = true;
        for(let i = 1; i <= 4; i++) {
            const type = this.querySelector(`input[name="link_type_${i}"]`).value.trim();
            const url = this.querySelector(`input[name="link_url_${i}"]`).value.trim();
        ...
        if(!validLinks) {
            e.preventDefault();
            alert('Please make sure all links have both type and URL filled, or leave both empty.');
            return;
        }
    ...
    linksContainer.addEventListener('input', function(e) {
        if(e.target.name.includes('link_type_') || e.target.name.includes('link_url_')) {
            const linkNumber = e.target.name.split('_')[2];
            const typeInput = document.querySelector(`input[name="link_type_${linkNumber}"]`);
            const urlInput = document.querySelector(`input[name="link_url_${linkNumber}"]`);
            
            if(typeInput.value.trim() && !urlInput.value.trim()) {
                urlInput.style.borderColor = '#dc3545';
            } else if(!typeInput.value.trim() && urlInput.value.trim()) {
